...
import { useAcceptTaskMutation, useAgentTasksQuery } from "@/hooks/agent/useAgentTasks";
import { formatDateDisplay } from "@/lib/agent.utils";

export default function AgentTaskAcceptPage() {
  ...
  return (
    <Card className="py-0">
      <CardHeader className="border-b px-4 py-3"><CardTitle>Accept Task</CardTitle></CardHeader>
      <CardContent className="space-y-3 px-4 py-4">
        ...
        <Button
          onClick={async () => {
            await acceptMutation.mutateAsync(task.taskId);
